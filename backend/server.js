...
const cloudOpsAgentPermissionModes = Object.freeze({
  on_request: {
    label: '请求批准',
...
async function getCloudOpsCodexDeploymentStatus() {
  ...
  return {
    ...
    cli,
    safety: [
      '所有山海智枢对话统一进入服务端智能体引擎，浏览器不直接持有模型凭据。',
      `实时搜索为 ${cloudOpsCodexWebSearchMode}；宿主机访问通过受控 cloud_shell。`,
      '车辆访问只通过后端 vehicle_tool / vehicle_ssh 动态工具，不允许本地 shell 绕过车辆安全网关。',
      '只读检查自动执行；自动审批和完全访问模式只对具备相应操作权限的账号开放。'
    ]
  };
}
...
async function hasYoloManualAnnotationOverrides(datasetId) {
...
function isYoloPublicDatasetSummary(summary) {
  if (!summary || typeof summary !== 'object') {
    return false;
  }
  return summary.source_type === 'public_dataset' || summary.public_dataset === true;
}
...
function yoloDatasetSourceTypeForSummary(summary, stats = {}) {
  ...
  if (isYoloPublicDatasetSummary(summary)) {
...
async function readYoloPatrolDatasetIndexSummary() {
  const delimiter = ',"rows":[';
  let handle = null;
  try {
    handle = await fs.open(yoloReviewPatrolIndexPath, 'r');
    const chunks = [];
    let totalBytes = 0;
    ...
    while (totalBytes < maxHeaderBytes) {
      ...
      const joined = Buffer.concat(chunks, totalBytes);
      const marker = joined.indexOf(delimiter);
      if (marker >= 0) {
        const headerText = `${joined.subarray(0, marker).toString('utf8')}}`;
        const payload = JSON.parse(headerText);
        const dataset = hydrateYoloPatrolDatasetFromIndex({ ...payload, rows: [] });
        if (dataset) {
          dataset.index_summary_only = true;
        }
        return dataset;
      }
    }
...
function yoloPatrolDatasetTotalImages(dataset) {
  const summaryCount = Number(dataset?.summary?.images?.patrol || 0);
  if (summaryCount > 0) {
    return summaryCount;
  }
...
async function resolveYoloPatrolDataset(options = {}) {
  ...
  if (summaryOnly) {
    ...
    const indexedSummary = await readYoloPatrolDatasetIndexSummary();
    if (indexedSummary) {
      if (yoloPatrolDatasetAgeMs(indexedSummary) >= yoloReviewPatrolIndexFreshMs) {
        scheduleYoloPatrolIndexRefresh('summary_file_stale');
      }
...
async function findFilesNamed(root, fileName, maxDepth = 4, depth = 0) {
  ...
  const matches = [];
  const skipLargeDatasetDirs = fileName === 'dataset_summary.json'
    ? new Set([
        'images',
        'labels',
        'qwen_labels',
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory() && depth < maxDepth) {
      if (skipLargeDatasetDirs?.has(entry.name)) {
        continue;
      }
...
async function buildYoloDatasetList() {
  ...
  try {
    const patrolDataset = await resolveYoloPatrolDataset({ summaryOnly: true });
    const indexedReviewQueue = Number(patrolDataset.summary?.qwen_bbox_audit?.review_queue_images);
    const patrolReviewQueue = Number.isFinite(indexedReviewQueue)
      ? indexedReviewQueue
      : await yoloPatrolReviewQueueCount(patrolDataset);
    const patrolAudit = {
    ...
    datasets.push({
      ...
      by_class_yes: null,
      max_task_id: null,
      total_images: yoloPatrolDatasetTotalImages(patrolDataset),
...
function yoloManifestClassList(value) {
  return [...new Set((Array.isArray(value) ? value : [])
    .map((item) => String(item || '').trim())
    .filter(Boolean))];
}

function yoloManifestLabelClassInfo(manifestItem, dataset) {
  if (!manifestItem || typeof manifestItem !== 'object') {
    return { classes: [], indexed: false };
  }
  if (Array.isArray(manifestItem.label_classes)) {
    return {
      classes: yoloManifestClassList(manifestItem.label_classes),
      indexed: true
    };
  }
  if (isYoloEventFeedbackSummary(dataset?.summary) && Array.isArray(manifestItem.independent_classes)) {
    return {
      classes: yoloManifestClassList(manifestItem.independent_classes),
      indexed: true
    };
  }
  return { classes: [], indexed: false };
...
function buildYoloBaseItem(dataset, rel, manifestItem = null, webReview = null) {
  ...
  const manifestLabelCount = Number(manifestItem?.label_count);
  const manifestBoxCount = Number(manifestItem?.box_count);
  const effectiveLabelCount = Number.isFinite(manifestLabelCount)
    ? manifestLabelCount
    : (Number.isFinite(manifestBoxCount) ? manifestBoxCount : undefined);
  ...
  return {
    ...
    image_rel_path: rel,
    split,
    ai_class: itemClass || manifestPrimaryClass,
    event_name: task?.event_name || metadata.event_name || itemClass || manifestPrimaryClass,
...
function yoloItemHasManifestClass(item, classNames) {
  if (!classNames.length) {
    return false;
  }
  return item?.label_classes_indexed === true && Array.isArray(item?.label_classes) && item.label_classes.some((value) => (
    classNames.includes(normalizeClassToken(value))
  ));
}
...
async function listYoloReviewItems(datasetId, query = {}) {
  ...
  const hasManualAnnotationOverrides = await hasYoloManualAnnotationOverrides(dataset.id);
  const canUseManifestLabelClassFilter = dataset.kind === 'detect' &&
    classNames.length > 0 &&
    hasBoxOnly &&
    !hasManualAnnotationOverrides &&
    !['YES', 'NO'].includes(aiAnswer) &&
    allItems.length > 0 &&
    allItems.every((item) => item.label_classes_indexed === true);
  const canPrefilterManifestLabelClass = dataset.kind === 'detect' &&
    classNames.length > 0 &&
    hasBoxOnly &&
    allItems.length > 0 &&
    allItems.every((item) => item.label_classes_indexed === true);
  const needsLabelBeforePagination = (
    ['YES', 'NO'].includes(aiAnswer) ||
    classNames.length > 0 ||
    hasBoxOnly
  ) && !canUseManifestLabelClassFilter;
  let items = qwenAudit
  ...
  items = items.filter((item) => {
    ...
    if (classNames.length && canUseManifestLabelClassFilter && Array.isArray(item.label_classes)) {
      if (!yoloItemHasManifestClass(item, classNames)) {
        return false;
      }
    }
    if (classNames.length && !canUseManifestLabelClassFilter && canPrefilterManifestLabelClass && Array.isArray(item.label_classes)) {
      if (!yoloItemHasManifestClass(item, classNames)) {
        return false;
      }
    }
...
function makeCloudOpsAgentVehicleSummary(vehicle) {
  ...
  const keyTopics = telemetry?.key_topics && typeof telemetry.key_topics === 'object'
    ? telemetry.key_topics
    : null;
  const keyNodes = telemetry?.key_nodes && typeof telemetry.key_nodes === 'object'
    ? telemetry.key_nodes
    : null;
  ...
  const summary = {
    ...
    key_topics_ok: keyTopics ? Object.values(keyTopics).filter(Boolean).length : null,
    key_topics_total: keyTopics ? Object.keys(keyTopics).length : null,
    key_topics_missing: keyTopics
      ? Object.entries(keyTopics).filter(([, ok]) => !ok).map(([name]) => name).slice(0, 20)
      : [],
    key_nodes_ok: keyNodes ? Object.values(keyNodes).filter(Boolean).length : null,
    key_nodes_total: keyNodes ? Object.keys(keyNodes).length : null,
    key_nodes_missing: keyNodes
      ? Object.entries(keyNodes).filter(([, ok]) => !ok).map(([name]) => name).slice(0, 20)
      : [],
    ...
    autodrive_blockers: Array.isArray(autodriveCheck?.blockers)
      ? autodriveCheck.blockers.map((item) => String(item)).slice(0, 20)
      : [],
    autodrive_warnings: Array.isArray(autodriveCheck?.warnings)
      ? autodriveCheck.warnings.map((item) => String(item)).slice(0, 20)
      : [],
...
const cloudOpsAgentFleetIssueLabels = Object.freeze({
  routing_route_available: '未检测到可用路线',
  control_chain_ready: '控制链未就绪',
  front_laser_topic_output: '前激光话题无输出',
  back_laser_topic_output: '后激光话题无输出',
  collision_beam_clear: '碰撞停已触发',
  emergency_stop_clear: '急停已触发',
  localization_ready: '定位未就绪',
  autodrive_nodes_online: '自动驾驶关键节点不完整',
  ultrasonic_clear: '超声波停障已触发',
  planning_estop_clear: '规划急停已触发'
});

function cloudOpsAgentFleetIssueDetail(code) {
  return cloudOpsAgentFleetIssueLabels[code] || String(code || '未知告警');
}

function inspectCloudOpsAgentVehicleIssues(summary) {
  const issues = [];
  const seen = new Set();
  const addIssue = (code, severity, detail) => {
    const normalizedCode = String(code || 'unknown');
    if (seen.has(normalizedCode)) return;
    seen.add(normalizedCode);
    issues.push({ code: normalizedCode, severity, detail });
  };

  if (isCloudOpsAgentVehicleStale(summary)) {
    addIssue('telemetry_stale', 'danger', '车辆心跳超过 5 分钟或缺少有效时间戳');
  }
  if (summary.master_ping_ok === false) {
    addIssue('master_unreachable', 'danger', 'media 无法连通主控');
  }
  if (summary.telemetry_master_ros_ok === false) {
    addIssue('master_ros_unhealthy', 'danger', '主控 ROS 状态异常');
  }
  if (summary.emergency_stop_pressed === true) {
    addIssue('emergency_stop_pressed', 'danger', '急停已触发');
  }
  if (summary.collision_stop === true) {
    addIssue('collision_stop', 'danger', '碰撞停已触发');
  }
  if (summary.localization_reliable === false) {
    addIssue('localization_unreliable', 'danger', '定位结果不可靠');
  }
  if (summary.trajectory_estop === true) {
    addIssue('planning_estop', 'danger', '规划轨迹处于急停状态');
  }
  if (summary.ready_for_autodrive === false) {
    addIssue('autodrive_not_ready', 'danger', '车辆未满足自动驾驶就绪条件');
  }

  summary.autodrive_blockers?.forEach((code) => {
    addIssue(`autodrive_blocker:${code}`, 'danger', cloudOpsAgentFleetIssueDetail(code));
  });
  summary.autodrive_warnings?.forEach((code) => {
    addIssue(`autodrive_warning:${code}`, 'warn', cloudOpsAgentFleetIssueDetail(code));
  });
  if (summary.autodrive_health && !['ok', 'healthy'].includes(String(summary.autodrive_health).toLowerCase())) {
    const severity = ['error', 'danger', 'critical', 'failed'].includes(String(summary.autodrive_health).toLowerCase())
      ? 'danger'
      : 'warn';
    addIssue('autodrive_health', severity, `自动驾驶健康状态为 ${summary.autodrive_health}`);
  }

  if (summary.key_nodes_total != null && Number(summary.key_nodes_ok) < Number(summary.key_nodes_total)) {
    const missing = summary.key_nodes_missing?.length ? `：${summary.key_nodes_missing.join('、')}` : '';
    addIssue('key_nodes_missing', 'warn', `关键节点在线 ${summary.key_nodes_ok}/${summary.key_nodes_total}${missing}`);
  }
  if (summary.key_topics_total != null && Number(summary.key_topics_ok) < Number(summary.key_topics_total)) {
    const missing = summary.key_topics_missing?.length ? `：${summary.key_topics_missing.join('、')}` : '';
    addIssue('key_topics_missing', 'warn', `关键话题正常 ${summary.key_topics_ok}/${summary.key_topics_total}${missing}`);
  }
  if (!summary.has_heartbeat || !summary.has_snapshot || !summary.has_telemetry) {
    addIssue('vehicle_cache_incomplete', 'warn', '车辆注册缓存缺少心跳、快照或遥测数据');
  }
  if (summary.stop_diagnosis?.severity === 'danger' || summary.stop_diagnosis?.label === '疑似停车故障') {
    addIssue('stop_diagnosis', summary.stop_diagnosis.severity === 'danger' ? 'danger' : 'warn',
      `${summary.stop_diagnosis.label}：${summary.stop_diagnosis.reason}`);
  }

  return {
    status: issues.length ? 'abnormal' : 'healthy',
    severity: issues.some((item) => item.severity === 'danger')
      ? 'danger'
      : issues.length
        ? 'warn'
        : 'ok',
...
function makeCloudOpsAgentFleetVehicleView(summary, detail = 'compact') {
  const inspection = inspectCloudOpsAgentVehicleIssues(summary);
  const lastSeenTimestamp = Date.parse(summary.last_seen || '');
  const base = {
    vehicle_id: summary.vehicle_id,
    status: inspection.status,
    severity: inspection.severity,
    last_seen: summary.last_seen,
    data_age_s: Number.isFinite(lastSeenTimestamp)
      ? Math.max(0, Math.floor((Date.now() - lastSeenTimestamp) / 1000))
      : null,
    autodrive_health: summary.autodrive_health,
    ready_for_autodrive: summary.ready_for_autodrive,
    master_ping_ok: summary.master_ping_ok,
    master_ros_ok: summary.telemetry_master_ros_ok,
    issues: inspection.issues
  };
  if (detail !== 'standard') return base;
  return {
    ...base,
    vehicle_ready: summary.vehicle_ready,
    speed_kph: summary.speed_kph,
    emergency_stop_pressed: summary.emergency_stop_pressed,
    collision_stop: summary.collision_stop,
    localization_reliable: summary.localization_reliable,
    planner_state: summary.planner_state,
    key_nodes: `${summary.key_nodes_ok ?? '?'}/${summary.key_nodes_total ?? '?'}`,
    key_topics: `${summary.key_topics_ok ?? '?'}/${summary.key_topics_total ?? '?'}`,
    blocker_count: summary.blocker_count,
    warning_count: summary.warning_count,
    stop_diagnosis: summary.stop_diagnosis
  };
}

function makeCloudOpsAgentFleetOverview(summaries = [], options = {}) {
  const detail = options?.detail === 'standard' ? 'standard' : 'compact';
  const onlyAbnormal = options?.only_abnormal !== false;
  const vehicles = summaries.map((summary) => makeCloudOpsAgentFleetVehicleView(summary, detail));
  const abnormalVehicles = vehicles.filter((vehicle) => vehicle.status === 'abnormal');
  const dangerCount = abnormalVehicles.filter((vehicle) => vehicle.severity === 'danger').length;
  const staleCount = summaries.filter((summary) => isCloudOpsAgentVehicleStale(summary)).length;
  return {
    ok: true,
    generated_at: new Date().toISOString(),
    source: 'cloud-agent 实时车辆注册缓存',
    fleet: {
      vehicle_count: vehicles.length,
      online_count: Math.max(0, vehicles.length - staleCount),
      stale_count: staleCount,
      healthy_count: vehicles.length - abnormalVehicles.length,
      abnormal_count: abnormalVehicles.length,
      danger_count: dangerCount,
      warning_count: abnormalVehicles.length - dangerCount,
      all_clear: vehicles.length > 0 && abnormalVehicles.length === 0
    },
    only_abnormal: onlyAbnormal,
    vehicles: onlyAbnormal ? abnormalVehicles : vehicles
  };
}

async function executeCloudOpsCodexFleetOverview(argumentsPayload = {}) {
  const summaries = await listCloudOpsAgentVehicleSummaries();
  return makeCloudOpsAgentFleetOverview(summaries, argumentsPayload);
}
...
function cloudOpsAgentHistoryRecord(record) {
  ...
  return {
    ...
    permission_mode: normalizeCloudOpsAgentPermissionMode(record.permission_mode)
  };
}
...
function normalizeCloudOpsAgentPermissionMode(value) {
  const mode = String(value || '').trim().toLowerCase();
  return Object.prototype.hasOwnProperty.call(cloudOpsAgentPermissionModes, mode) ? mode : 'on_request';
}

function cloudOpsAgentPermissionConfig(value) {
  const mode = normalizeCloudOpsAgentPermissionMode(value);
  return { mode, ...cloudOpsAgentPermissionModes[mode] };
}
...
function isCloudOpsCodexForbiddenVehicleTool(toolName, args) {
  const name = String(toolName || '').trim().toLowerCase();
  if (
    /^(?:can\.(?:send|raw)|vehicle\.(?:drive|motion)|chassis\.(?:command|control)|remote_control(?:\.|$))/.test(name)
  ) {
    return true;
  }
  const serializedArgs = safeJsonStringify(args || {}, 6000);
  return /\b(?:cansend|cmd_vel|vehicleControl|chassis_command|motion_control|bypass_(?:estop|safety))\b/i.test(serializedArgs);
}

function isCloudOpsCodexAutoReviewHighRisk(kind, detail) {
  const command = String(detail?.command || '').trim();
  if (command && (
    /(^|[;&|]\s*)(?:sudo\s+)?(?:rm|shred|wipefs|mkfs|fdisk|parted|dd)(?:\s|$)/i.test(command) ||
    /\b(?:shutdown|poweroff|halt|init\s+[06]|userdel|groupdel|passwd|chpasswd|iptables|nft|ufw)\b/i.test(command) ||
    /(?:^|\s)(?:chmod|chown)\s+-R\b[^\n]*(?:\s\/|\s\/etc|\s\/home)(?:\s|$)/i.test(command) ||
    /(?:>|tee\s+)(?:\s*)(?:\/etc\/(?:passwd|shadow|sudoers)|\/root\/\.ssh)/i.test(command)
  )) {
    return true;
  }
  if (kind === 'vehicle_tool') {
    return isCloudOpsCodexForbiddenVehicleTool(detail?.tool_name, detail?.arguments);
  }
  return false;
}
...
function isCloudOpsCodexReadOnlySshCommand(command) {
  ...
  if (commandName === 'ping') {
    ...
    for (let index = 1; index < parts.length; index += 1) {
      ...
      if (part === '-n') continue;
      if (part === '-c' || part === '-W' || part === '-w') {
        const value = Number(parts[index + 1]);
        const maxValue = part === '-w' ? 15 : 5;
        if (!Number.isInteger(value) || value < 1 || value > maxValue) return false;
        index += 1;
        continue;
      }
...
function buildCloudOpsCodexInstructions({ vehicleId, summary, fleetOverview, tailscalePeer, permissionMode }) {
  const permission = cloudOpsAgentPermissionConfig(permissionMode);
  const permissionInstruction = permission.mode === 'on_request'
    ? '写入、重启、部署和非白名单命令必须暂停并等待网页审批。'
    : permission.mode === 'auto_review'
      ? '变更由安全审查器根据风险代为审批；被拒绝时不得绕过。'
      : '用户已授予完全访问权限；可在完成需求所必需的范围内直接执行变更，但车辆安全硬限制仍不可绕过。';
  return [
    '你是“山海智枢”，由 GPT-5.6 Terra 统一驱动。你不是简单问答接口，而是可以自主规划、搜索、读取文件、执行 shell、调用车辆工具并完成变更的通用智能体。',
    ...
    '使用内置 shell 读取项目文件和执行命令；需要读取宿主机 systemd、Docker、端口、进程或日志时使用 cloud_shell。严格遵循当前权限模式，操作被拒绝时不得声称已执行。',
    '车辆问题必须先调查再变更。车辆实时数据和远程操作只允许使用 vehicle_tool 或 vehicle_ssh 动态工具；不要用本地 curl、ssh、scp 绕过车辆安全网关。',
    '当用户询问“哪辆车有问题”“全部车辆是否正常”“车队状态/告警/在线情况”等未指定单车的问题时，必须先调用 fleet_overview 获取最新车队快照。不得要求用户先提供车辆编号，也不得把未指定目标车辆解释为没有可查询车辆。',
    'fleet_overview 返回 abnormal_count > 0 时，直接列出每台异常车辆及 issues 证据；all_clear=true 时明确回答当前已注册车辆均未发现异常；vehicle_count=0 时才说明当前确实没有注册车辆。需要深入某台异常车时，再调用 vehicle_tool 或 vehicle_ssh。',
    `vehicle_tool 和 vehicle_ssh 的严格只读操作会自动执行。${permissionInstruction}`,
    '不得把密码、Token、私钥或 Cookie 放进命令。',
    ...
    '除非用户明确询问系统实现或运行时诊断，最终回复不得提及 Codex、OpenAI、App Server、CLI 或其他底层产品名，统一以“山海智枢”的身份回复。',
    `当前权限模式：${permission.label}。`,
    `当前服务器时间：${new Date().toISOString()}`,
    `当前目标车辆：${vehicleId || '未指定'}`,
    `车辆注册摘要：${safeJsonStringify(summary, 8000)}`,
    `任务开始时的车队异常快照：${safeJsonStringify(fleetOverview, 12000)}`,
    `Tailscale 线索：${safeJsonStringify(tailscalePeer, 2000)}`
  ].join('\n');
}

async function runCloudOpsCodexAgent({ message, requestedModel, history, vehicleId, actor, actorUser, permissionMode, operationIntent, onProgress }) {
  ...
  const fleetSummaries = dedupeCloudOpsAgentVehicles(vehicles).map(makeCloudOpsAgentVehicleSummary);
  const fleetOverview = makeCloudOpsAgentFleetOverview(fleetSummaries, { only_abnormal: true, detail: 'compact' });
  ...
  let lastReasoningEmissionLength = 0;
  ...
  const requestCustomApproval = async (kind, detail) => {
    const requiredPermission = detail.required_permission || 'vehicle:control';
    if (permission.mode !== 'on_request') {
      const hasPermission = authStore.hasPermission(actorUser, requiredPermission);
      const riskDenied = permission.mode === 'auto_review' && isCloudOpsCodexAutoReviewHighRisk(kind, detail);
      const approved = hasPermission && !riskDenied;
      cloudOpsAgentProgress(onProgress, {
        stage: 'permission_review',
        status: approved ? 'completed' : 'declined',
        title: approved
          ? `${permission.label}已放行本次操作`
          : riskDenied
            ? '安全审查已拒绝高风险操作'
            : '当前账号无权执行本次操作',
        detail: detail.command || detail.tool_name || requiredPermission,
        elapsed_ms: Date.now() - startedAt
      });
      return approved;
    }
  ...
  const handleDynamicToolCall = async (messagePayload) => {
    ...
    try {
      if (params.tool === 'cloud_shell') {
        ...
        if (!autoRead) {
          const approved = await requestCustomApproval('cloud_shell', {
            title: '确认执行云服务器命令',
            reason: args.reason || '山海智枢请求修改云服务器状态',
            command,
            cwd: cloudOpsCodexAgentCwd,
        ...
        cloudOpsAgentProgress(onProgress, {
          ...
          status: 'running',
          title: autoRead ? '正在执行宿主机只读命令' : '正在执行已授权的宿主机命令',
      ...
      if (params.tool === 'fleet_overview') {
        cloudOpsAgentProgress(onProgress, {
          stage: 'codex_fleet_overview',
          status: 'running',
          title: '正在读取实时车队概览',
          detail: args.only_abnormal === false ? '检查全部已注册车辆' : '筛选当前异常车辆',
          elapsed_ms: Date.now() - startedAt
        });
        const result = await executeCloudOpsCodexFleetOverview(args);
        cloudOpsAgentProgress(onProgress, {
          stage: 'codex_fleet_overview',
          status: 'completed',
          title: `已检查 ${result.fleet.vehicle_count} 台车辆`,
          detail: result.fleet.abnormal_count
            ? `发现 ${result.fleet.abnormal_count} 台异常车辆`
            : '当前已注册车辆未发现异常',
          elapsed_ms: Date.now() - startedAt
        });
        respond(messagePayload.id, cloudOpsCodexDynamicToolResponse(true, result));
        return;
      }
      if (params.tool === 'vehicle_tool') {
        const toolName = String(args.tool_name || '').trim();
        if (isCloudOpsCodexForbiddenVehicleTool(toolName, args.args)) {
          respond(messagePayload.id, cloudOpsCodexDynamicToolResponse(false, {
            ok: false,
            error: 'tool_blocked_by_vehicle_safety_policy'
          }));
          return;
        }
        ...
        if (!autoRead) {
          const approved = await requestCustomApproval('vehicle_tool', {
            ...
            reason: args.reason || '山海智枢根据诊断提出车辆变更',
      ...
      if (params.tool === 'vehicle_ssh') {
        ...
        if (!autoRead) {
          const approved = await requestCustomApproval('vehicle_ssh', {
            ...
            reason: args.reason || '山海智枢根据诊断提出远程修复命令',
            vehicle_id: args.vehicle_id || vehicleId,
            command
          });
          if (!approved) {
  ...
  const handleNotification = (messagePayload) => {
    ...
    if (messagePayload.method === 'item/reasoning/summaryTextDelta') {
      ...
      if (reasoningSummary.length - lastReasoningEmissionLength >= 80) {
        lastReasoningEmissionLength = reasoningSummary.length;
        cloudOpsAgentProgress(onProgress, {
          stage: 'codex_reasoning',
          status: 'running',
          title: '正在深度思考',
          detail: normalizeReply(reasoningSummary).slice(-4000),
          elapsed_ms: Date.now() - startedAt
        });
      }
      return;
    ...
    if (messagePayload.method === 'item/started' || messagePayload.method === 'item/completed') {
      ...
      if (item.type === 'webSearch') {
        ...
        cloudOpsAgentProgress(onProgress, {
          ...
          title: messagePayload.method === 'item/completed' ? '实时网页检索已完成' : '正在检索实时信息',
      ...
      if (item.type === 'commandExecution') {
        cloudOpsAgentProgress(onProgress, {
          stage: 'codex_tool',
          status: messagePayload.method === 'item/completed' ? 'completed' : 'running',
          title: messagePayload.method === 'item/completed' ? '命令已完成' : '正在执行只读命令',
    ...
    if (messagePayload.method === 'turn/completed') {
      completed = true;
      if (reasoningSummary.length > lastReasoningEmissionLength) {
        lastReasoningEmissionLength = reasoningSummary.length;
        cloudOpsAgentProgress(onProgress, {
          stage: 'codex_reasoning',
          status: 'completed',
  ...
  try {
    cloudOpsAgentProgress(onProgress, {
      ...
      status: 'running',
      title: '正在启动山海智枢任务',
      detail: `${codexModel} · ${cloudOpsAgentReasoningEffort} · 实时搜索 ${cloudOpsCodexWebSearchMode} · ${permission.label}`,
      elapsed_ms: 0
    });
    ...
    const thread = await request('thread/start', {
      ...
      dynamicTools: [
        {
          name: 'fleet_overview',
          description: '读取 cloud-agent 最新车队注册与健康快照。用于回答哪辆车有问题、全部车辆是否正常、车队告警和在线情况；无需预先指定车辆。默认只返回异常车辆。',
          inputSchema: {
            type: 'object',
            properties: {
              only_abnormal: { type: 'boolean', description: 'true 时只返回异常车辆；false 时返回全部车辆。' },
              detail: { type: 'string', enum: ['compact', 'standard'], description: 'compact 返回核心异常证据；standard 增加运行状态和节点话题统计。' }
            },
        ...
        {
          name: 'cloud_shell',
          description: '在当前云服务器宿主机执行命令。hostname、uptime、systemctl status/is-active、journalctl、docker ps/logs 等严格只读命令自动执行；其他操作按当前权限模式处理。禁止携带密码、Token、Cookie 或私钥。',
          inputSchema: {
            type: 'object',
        ...
        {
          name: 'vehicle_tool',
          description: '调用 cloud-agent 注册的车辆工具。只读工具自动执行；变更工具按当前权限模式和账号权限处理。',
          inputSchema: {
            type: 'object',
        ...
        {
          name: 'vehicle_ssh',
          description: '通过后端受控 SSH 进入车辆 media 或主控。严格只读命令自动执行，其他命令按当前权限模式处理。禁止携带密码、Token 或私钥。',
          inputSchema: {
            type: 'object',
    ...
    return {
      ok: true,
      answer: normalizeReply(answer) || '山海智枢已完成任务，但没有返回最终文本。',
      model: codexModel,
      ...
      context: {
        ...
        fleet_vehicle_count: fleetOverview.fleet.vehicle_count,
        fleet_abnormal_count: fleetOverview.fleet.abnormal_count,
        web_search_count: webSearchCount
      },
      reasoning_summary: normalizeReply(reasoningSummary).slice(-6000)
    };
  } finally {
    closing = true;
  ...
  try {
    ...
    for (let recordStart = 0; recordStart < totalRecords; recordStart += recordsPerChunk) {
      ...
      for (let recordIndex = firstOffset; recordIndex < recordsRead; recordIndex += sampleStride) {
        ...
        const x = readLidarRelocPcdField(chunk, recofset, xField);
...
const MAP_EDITOR_ROUTE_EDIT_BODY_TARGET_BYTES = 4 * 1024 * 1024;
const MAP_EDITOR_ROUTE_EDIT_CLOUD_AGENT_BODY_TARGET_BYTES = 4 * 1024 * 1024;
...
app.get('/api/cloud-ops-agent/status', authStore.requirePermission('vehicle:read'), async (req, res) => {
  ...
  return res.json({
    ...
    permission_modes: Object.entries(cloudOpsAgentPermissionModes).map(([value, item]) => ({
      value,
      label: item.label,
      requires_control: value !== 'on_request'
    })),
    can_use_autonomous_permissions: authStore.hasPermission(req.jgzjAuth?.user, 'vehicle:control'),
    ...
    safeguards: [
      ...
      '所有用户消息统一由 GPT-5.6 Terra 智能体引擎执行；模型凭据不下发到浏览器。',
      `时效性问题可调用实时搜索（${cloudOpsCodexWebSearchMode}），结果必须给出可核验来源。`,
      '具体车辆状态问题由山海智枢自主调用实时只读工具；注册缓存缺失时可补充 Tailscale 和 SSH 只读巡检。',
      '请求批准模式下，写入、重启、部署和非白名单 SSH 命令逐项请求网页审批。',
...
app.post('/api/cloud-ops-agent/run', authStore.requirePermission('vehicle:read'), async (req, res) => {
  ...
  const permissionMode = normalizeCloudOpsAgentPermissionMode(req.body?.permission_mode);
  const permission = cloudOpsAgentPermissionConfig(permissionMode);
  ...
  if (permissionMode !== 'on_request' && !authStore.hasPermission(req.jgzjAuth?.user, 'vehicle:control')) {
    return res.status(403).json({
      ok: false,
  ...
  try {
    ...
    sendEvent('progress', {
      ...
      status: 'completed',
      title: runCodex
        ? `${codexRequested ? '自动进入智能运维' : '山海智枢正在处理'}${vehicleId ? ` · ${vehicleId}` : ''}`
        : runDiagnosis
          ? `自动进入 ${vehicleId} 车辆诊断`
          : '自动进入一体化对话',
      detail: runCodex
        ? codexRequested
          ? `山海智枢将自主调查并调用所需工具 · ${permission.label}`
          : `GPT-5.6 Terra · 实时搜索 ${cloudOpsCodexWebSearchMode} · ${cloudOpsAgentReasoningEffort} · ${permission.label}`
        : runDiagnosis
