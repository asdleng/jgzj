...
最近检查时识别进度：106 个采集点已完成识别，四路合计 265 人，单点峰值 29 人；后续新增采集点会在 GPU 空闲时继续自动处理，具体数值以 `crowd-analysis-state.json` 和页面接口为准。
...
- 地图始终是单车历史轨迹视图，不把所有车辆采集点挤在同一张地图上，避免跨区域自动缩放到全国尺度。
- 地图显示历史采集轨迹、采集点和人数热力层；热力权重只使用已识别的 `people_count`，不使用图片数量。
- 热力层会在同车相邻采集点之间按轨迹距离插值铺开，形成连续二维色带；超过 750m 的跳点不连接。
- 采集点列表与地图联动。
...
5. 当前页面只画单车热力层，切车后重新按该车历史轨迹和人数生成热力。
6. 同车相邻采集点距离不超过 750m 时按 28m 间隔插值热力点，连接成连续热力带；更远距离视为跳点，不做插值连接。
7. 对高人流点建议用 A100 `Qwen3.6-27B-MM` 做抽检复核，再进入广告引流分析。

## 后续人群特征

下一步可以在人数识别结果里追加 `age_groups` 和 `crowd_traits`：

- `children_count`：儿童/小孩数量估计。
- `elderly_count`：老人数量估计。
- `adult_count`：成年人人数估计。
- `family_group_count`：亲子/家庭同行组估计。
- `group_density`：稀疏、一般、拥挤。

这类特征比总人数更容易受角度、遮挡和鱼眼畸变影响，建议先用本机模型粗筛，高人流或广告价值点再用 A100 多模态模型复核。
