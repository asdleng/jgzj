...
- 截至本次检查：已有 105 条采集样本，图片和 sidecar 元数据文件共 840 个。
- 最近一次监控：扫 16 台，确认巡逻 2 台，BIT-0033 成功采集 4 路图约 1.26MB。
...
- `GET /api/park-pcm/crowd/vehicles/:vehicle_id/detail`：单车定位、巡逻状态、采样统计。
- `GET /api/park-pcm/crowd/patrols`：当前确认巡逻车辆。
- `GET /api/park-pcm/crowd/samples`：采集样本，自动合并人数分析结果。
