...
DEDUPE_EXACT="${YOLO_EVENT_FEEDBACK_DEDUPE_EXACT:-1}"
DEDUPE_NEAR="${YOLO_EVENT_FEEDBACK_DEDUPE_NEAR:-0}"
DEDUPE_WINDOW_SECONDS="${YOLO_EVENT_FEEDBACK_DEDUPE_WINDOW_SECONDS:-60}"
DEDUPE_PHASH_THRESHOLD="${YOLO_EVENT_FEEDBACK_DEDUPE_PHASH_THRESHOLD:-6}"
DEDUPE_DHASH_THRESHOLD="${YOLO_EVENT_FEEDBACK_DEDUPE_DHASH_THRESHOLD:-10}"
DEDUPE_MAX_NEIGHBORS="${YOLO_EVENT_FEEDBACK_DEDUPE_MAX_NEIGHBORS:-12}"

DEDUPE_EXACT_ARG="--dedupe-exact"
if [ "$DEDUPE_EXACT" = "0" ]; then
  DEDUPE_EXACT_ARG="--no-dedupe-exact"
fi
DEDUPE_NEAR_ARG="--dedupe-near"
if [ "$DEDUPE_NEAR" = "0" ]; then
  DEDUPE_NEAR_ARG="--no-dedupe-near"
fi
...

{
  ...
  echo "[$(timestamp)] start yolo_event_feedback_sync days=$DAYS dedupe_exact=$DEDUPE_EXACT dedupe_near=$DEDUPE_NEAR window=${DEDUPE_WINDOW_SECONDS}s phash=$DEDUPE_PHASH_THRESHOLD dhash=$DEDUPE_DHASH_THRESHOLD max_neighbors=$DEDUPE_MAX_NEIGHBORS"
  ionice -c2 -n7 nice -n 10 python3 scripts/build_yolo_event_feedback_dataset.py \
    ...
    "$DEDUPE_EXACT_ARG" \
    "$DEDUPE_NEAR_ARG" \
    --dedupe-window-seconds "$DEDUPE_WINDOW_SECONDS" \
    --dedupe-phash-threshold "$DEDUPE_PHASH_THRESHOLD" \
    --dedupe-dhash-threshold "$DEDUPE_DHASH_THRESHOLD" \
    --dedupe-max-neighbors "$DEDUPE_MAX_NEIGHBORS"
