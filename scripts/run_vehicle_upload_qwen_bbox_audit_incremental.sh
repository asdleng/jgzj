...
LOCK="$RUNTIME/vehicle_upload_qwen_bbox_audits_v1.lock"
LOG="$RUNTIME/vehicle_upload_qwen_bbox_audits_v1.incremental.log"
RUN_OUTPUT="$(mktemp "$RUNTIME/vehicle_upload_qwen_bbox_audits_v1.run.XXXXXX")"
...
MAX_NEW="${VEHICLE_QWEN_AUDIT_MAX_NEW:-1000}"
...

{
  ...
  echo "[$(timestamp)] start vehicle_upload_qwen_bbox_audit max_new=$MAX_NEW workers=$WORKERS class_filter=$CLASS_FILTER"
  PROMPT_ARGS=()
  ...
  echo "[$(timestamp)] done vehicle_upload_qwen_bbox_audit"

  if grep -Eq 'candidate_rows=0[[:space:]]+rows=0' "$RUN_OUTPUT"; then
    echo "[$(timestamp)] skip patrol_dataset_index_refresh: no new audit rows"
    exit 0
  fi
