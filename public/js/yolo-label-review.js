(() => {
  ...
  const sourceGroups = [
    ...
    { value: "finetune_dataset", label: "二次整合数据集" },
  ...
  function resetDetail(message = "选择一条训练样本。") {
    if (typeof state.detailCleanup === "function") {
      state.detailCleanup();
      state.detailCleanup = null;
    }
  ...
  function syncOverlayToImage(img, overlay) {
    if (!img || !overlay) return;
    const boxWidth = img.clientWidth || img.offsetWidth || 0;
    const boxHeight = img.clientHeight || img.offsetHeight || 0;
  ...
  function renderDetail(dataset, item) {
    ...
    const syncOverlay = () => syncOverlayToImage(img, overlay);
    let overlayResizeObserver = null;
    img.addEventListener("load", syncOverlay);
    if (img.complete) requestAnimationFrame(syncOverlay);
    window.addEventListener("resize", syncOverlay, { passive: true });
    if ("ResizeObserver" in window) {
      overlayResizeObserver = new ResizeObserver(syncOverlay);
      overlayResizeObserver.observe(img);
      overlayResizeObserver.observe(panContent);
    }
    state.detailCleanup = () => {
