...
def select_nohard_pool_samples(
    ...
    if args.positive_total <= 0 and args.negative_total <= 0:
        raise SystemExit("--positive-total or --negative-total must be > 0 for no-hard pool sampling.")
    primary, primary_summary = build_dataset_pool_source(
        role="primary",
        value=args.primary_dataset,
        dataset_id=args.primary_dataset_id,
    ...
    positive_requested = int(args.positive_total)
    negative_requested = int(args.negative_total)
...
def main() -> None:
    ...
    nohard_pool_mode = args.positive_total > 0 or args.negative_total > 0
    model = None
